...
void Material::bind(GFX::Shader* shader) {
	...
	// Bind the textures and set uniforms =======================
	{
		...
		if (textures[SCN::eTextureChannel::METALLIC_ROUGHNESS].texture == textures[SCN::eTextureChannel::NORMALMAP].texture) {
			int x = 1;
		}
		...
		if(metallic_roughness_texture == NULL)
			metallic_roughness_texture = GFX::Texture::getWhiteTexture();
		...
		if (metallic_roughness_texture)
			shader->setUniform("u_metallic_roughness_texture", metallic_roughness_texture);
